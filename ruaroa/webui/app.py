...
logger = logging.getLogger("ruaroa")
...
class WebUI:
    """
    WebUI encapsulates the Streamlit application logic for the Ruaroa AI Zero-Code Machine Learning Wizard.
    """
    ...
    def setup_page():
        ...
        """
        st.set_page_config(
            page_title="Ruaroa AI: Zero-Code Machine Learning Wizard",
            layout="wide",
        )
        WebUI.load_css()
    ...
    def run(self):
        ...
        """
        # Add Ruaroa AI Header with Modern Logo
        st.markdown(
            """
            <div class="ruaroa-header">
                <div class="ruaroa-logo-container">
                    <div class="ruaroa-logo">
                ...
                <div class="ruaroa-brand-text">
                    <h1 class="ruaroa-title">Ruaroa AI</h1>
                    <p class="ruaroa-subtitle">Zero-Code Machine Learning Wizard</p>
                </div>
            </div>
            """,
    ...
    def render_input_section(self, results_col):
        ...
        if st.button("Run Ruaroa AI", type="primary", use_container_width=True):
            with st.spinner("Ruaroa AI is running..."):
                results = self.run_ruaroa(
    ...
    def handle_file_upload(self):
        ...
        if not st.session_state.get("example_files"):
            ...
            if st.button(
                "Try Sample Dataset", type="primary", use_container_width=True
            ):
                st.session_state.example_files = self.load_example_files()

        if st.session_state.get("example_files"):
            st.info("Example files loaded! Click 'Run Ruaroa AI' to proceed.")
    ...
    def handle_user_inputs(self):
        ...
        goal_text = st.text_area(
            ...
            placeholder="Example: Estimate property values for residential homes",
            help="Describe what you want the AI to accomplish with your data"
        )
        eval_text = st.text_area(
            "📏 Success Metrics",
            value=st.session_state.get("eval", ""),
            placeholder="Example: Apply Root Mean Square Error on log-transformed predictions vs actual values.",
            help="Specify how the AI should measure the quality of its solutions"
        )
        num_steps = st.slider(
            "⚡ Iteration Count",
            min_value=1,
            ...
            help="Number of optimization iterations the AI will perform"
        )
    ...
    def load_example_files():
        ...
        st.session_state["goal"] = "Estimate property values for residential homes"
        st.session_state["eval"] = (
            "Apply Root Mean Square Error on log-transformed predictions vs actual values."
        )

        return example_files

    def run_ruaroa(self, files, goal_text, eval_text, num_steps, results_col):
        """
        Run the Ruaroa AI experiment with the provided inputs.
        ...
        try:
            ...
            for step in range(num_steps):
                ...
                with progress_placeholder.container():
                    st.markdown(
                        f"### ⚡ Processing Iteration {st.session_state.current_step}/{st.session_state.total_steps}"
                    )
    ...
    def initialize_experiment(input_dir, goal_text, eval_text):
        """
        Initialize the Ruaroa AI Experiment.
